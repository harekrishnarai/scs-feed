...
// Helper function to convert GitLab repository URLs to API URLs
function convertGitLabUrlToApi(gitlabUrl) {
  const url = new URL(gitlabUrl);
  const baseUrl = `${url.protocol}//${url.host}`;
  const projectPath = url.pathname.replace(/^\//, '').replace(/\.git$/, ''); // Remove leading slash and .git suffix
  const encodedPath = encodeURIComponent(projectPath);
  
  return `${baseUrl}/api/v4/projects/${encodedPath}/repository/commits`;
}
...
async function generateDailyReport() {
  ...
  for (const source of apiSources) {
    ...
    if (source.type === 'github') {
      ...
      const reports = await fetchGitLabCommits(source);
      allReports = allReports.concat(reports);
